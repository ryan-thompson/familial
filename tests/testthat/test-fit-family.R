...
test_that('first and last points of huber family are mean and median for even samples', {
  ...
  expect_equal(head(path$mu.hat, 1), mean(x))
  expect_equal(tail(path$mu.hat, 1), median(x))
  w <- abs(rnorm(100))
  path.w <- fit.family(x, w, family = 'huber')
  expect_equal(head(path.w$mu.hat, 1), weighted.mean(x, w))
  expect_equal(tail(path.w$mu.hat, 1), weighted.median(x, w))
})
...
test_that(paste('first and last points of huber family are mean and median for samples with repeated observations', .Machine$double.eps), {
  set.seed(123)
  x <- rnorm(10)
  x <- rep(x, 10)
  path <- fit.family(x, family = 'huber')
