...
An R package for familial inference. A paper describing familial inference is forthcoming. Briefly, this package provides tests for hypotheses of the form
$$
\begin{aligned}
...
\end{aligned}
$$
where $\{\mu(\lambda):\lambda\in\Lambda\}$ is a family of centers, e.g., that induced by the Huber loss function with parameter $\lambda$. In contrast to classic statistical tests such as the $t$ or sign tests for the mean or median, familial tests do not depend on a single (sometimes arbitrarily chosen) measure of center. See the package [vignette](vignette.pdf) for illustrations.
